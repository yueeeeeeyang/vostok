...
import { useVkTable } from './use-vk-table';
import type {
  VkAdvancedFilterValue,
...
const props = withDefaults(defineProps<VkTableProps<T>>(), {
...
const { loading, items, total, error, load } = useVkTable<T>(props.fetcher);
const page = ref(1);
const pageSize = ref(props.defaultPageSize);
const keyword = ref('');
const advancedExpanded = ref(false);
const sortKey = ref('');
const sortOrder = ref<VkTableSortOrder>(null);
const visibleColumnKeys = ref<string[]>([]);
const advancedFilters = reactive<Record<string, VkAdvancedFilterValue>>({});
...
function resolveFieldType(field: VkAdvancedSearchField): 'input' | 'select' | 'datetime_range' {
  return field.type ?? 'input';
}

function getDefaultAdvancedValue(field: VkAdvancedSearchField): VkAdvancedFilterValue {
  return resolveFieldType(field) === 'datetime_range' ? null : '';
}

function toInputFilterValue(value: VkAdvancedFilterValue): string {
...
function toSelectFilterValue(value: VkAdvancedFilterValue): string | number | null {
  if (typeof value === 'string') {
...
function toSelectOptions(
  options: VkAdvancedSearchField['options'] | undefined
): SelectOption[] {
...
function toDateRangeFilterValue(value: VkAdvancedFilterValue): [number, number] | null {
...
function hasAdvancedFilterValue(value: VkAdvancedFilterValue): boolean {
  if (typeof value === 'string') {
    return value.trim() !== '';
  }
...
const selectedAdvancedFilters = computed<Record<string, VkAdvancedFilterValue>>(() => {
  const result: Record<string, VkAdvancedFilterValue> = {};
...
function setAdvancedFilter(key: string, value: VkAdvancedFilterValue): void {
...
function buildQuery(): VkTableQuery {
  return {
    page: page.value,
    pageSize: pageSize.value,
...
.suffix-btn {
  ...
  height: 18px;
  border-radius: 999px;
  color: var(--n-placeholder-color, #9ca3af);
  cursor: pointer;
}
.suffix-btn:hover {
  background: var(--n-action-color, #f3f4f6);
