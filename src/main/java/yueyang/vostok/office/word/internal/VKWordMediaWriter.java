...
/** 写入 Word 包中的 media 文件。 */
public final class VKWordMediaWriter {
    private static final int BUFFER_SIZE = 64 * 1024;

    private final Path packageRoot;
    private final Path baseDir;

    public VKWordMediaWriter(Path packageRoot, Path baseDir) {
    ...
    public PreparedImage writeBytes(int imageIndex, String fileName, byte[] bytes) {
        if (bytes == null || bytes.length == 0) {
            throw new VKOfficeException(VKOfficeErrorCode.INVALID_ARGUMENT, "Word image bytes is empty");
        }

        String safeName = VKWordSecurityGuard.safeFileName(fileName, "image" + imageIndex + ".bin");
        String ext = VKWordContentTypeResolver.extension(safeName);
        String mediaPart = "word/media/image" + imageIndex + "." + ext;
        ...
        try {
            ...
            throw new VKOfficeException(VKOfficeErrorCode.WRITE_ERROR,
                    "Write word image bytes failed: " + mediaPart, e);
        }

        return new PreparedImage(mediaPart, ext,
                VKWordContentTypeResolver.contentTypeByFileName(safeName), bytes.length);
    }

    public PreparedImage writeFile(int imageIndex, String filePath) {
        if (filePath == null || filePath.isBlank()) {
            throw new VKOfficeException(VKOfficeErrorCode.INVALID_ARGUMENT, "Word image file path is blank");
        }
        try {
            VostokSecurity.assertSafePath(filePath);
        } catch (Exception e) {
            throw new VKOfficeException(VKOfficeErrorCode.SECURITY_ERROR,
                    "Unsafe word image file path: " + filePath, e);
        }

        Path source = resolveAgainstBase(filePath);
        if (!Files.exists(source) || !Files.isRegularFile(source)) {
            throw new VKOfficeException(VKOfficeErrorCode.NOT_FOUND,
                    "Word image file not found: " + filePath);
        }

        String ext = VKWordContentTypeResolver.extension(source.getFileName().toString());
        String mediaPart = "word/media/image" + imageIndex + "." + ext;
        Path target = packageRoot.resolve(mediaPart).normalize();
        ensureTargetInPackage(target, mediaPart);
        ...
        try {
            ...
            return new PreparedImage(mediaPart, ext,
                    VKWordContentTypeResolver.contentTypeByFileName(source.getFileName().toString()), size);
        } catch (VKOfficeException e) {
            throw e;
        } catch (Exception e) {
            throw new VKOfficeException(VKOfficeErrorCode.WRITE_ERROR,
                    "Copy word image file failed: " + source + " -> " + mediaPart, e);
        }
    }
    ...
    private Path resolveAgainstBase(String rawPath) {
        ...
        if (!resolved.startsWith(baseDir)) {
            throw new VKOfficeException(VKOfficeErrorCode.SECURITY_ERROR,
                    "Word image file path escapes file baseDir: " + rawPath);
        }
        return resolved;
    }
    ...
    private void ensureTargetInPackage(Path target, String mediaPart) {
        if (!target.startsWith(packageRoot.normalize())) {
            throw new VKOfficeException(VKOfficeErrorCode.SECURITY_ERROR,
                    "Word media path escapes package root: " + mediaPart);
        }
    }
