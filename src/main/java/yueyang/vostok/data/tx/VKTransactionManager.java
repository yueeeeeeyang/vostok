package yueyang.vostok.data.tx;
...
public final class VKTransactionManager {
    ...
    public static void beginRequired(VKDataSource dataSource, VKTxIsolation isolation, boolean readOnly, VKTxPropagation propagation, boolean savepointEnabled) {
        ...
        switch (propagation) {
            ...
            case MANDATORY:
                if (ctx.conn == null) {
                    throw new yueyang.vostok.data.exception.VKTxException("No existing transaction for MANDATORY");
                }
                ctx.depth++;
                return;
            case NEVER:
                if (ctx.conn != null) {
                    throw new yueyang.vostok.data.exception.VKTxException("Existing transaction found for NEVER");
                }
                ctx.nonTxDepth++;
                return;
            case REQUIRED:
    ...
    public static void commit() {
        ...
        try {
            ...
            rollback();
            throw new yueyang.vostok.data.exception.VKTxException("Failed to commit", e);
        } finally {
            if (ctx.conn != null) {
                cleanup(ctx);
            }
    ...
    public static void rollback() {
        ...
        try {
            ...
            throw new yueyang.vostok.data.exception.VKTxException("Failed to rollback", e);
        } finally {
            cleanup(ctx);
        }
    }
        ...
        if (ctx.depth > 1) {
            if (ctx.savepointEnabled && !ctx.savepoints.isEmpty()) {
                ...
                try {
                    ...
                    throw new yueyang.vostok.data.exception.VKTxException("Failed to rollback to savepoint", e);
                } finally {
                    ctx.depth--;
                }
                return;
    ...
    public static Connection getConnection() {
        ...
        if (ctx == null || ctx.conn == null) {
            throw new yueyang.vostok.data.exception.VKTxException("No active transaction");
        }
        return ctx.conn;
    }
    ...
    private static Connection openConn(VKDataSource dataSource, VKTxIsolation isolation, boolean readOnly) {
        try {
            ...
            throw new yueyang.vostok.data.exception.VKTxException("Failed to open transaction connection", e);
        }
    }
    ...
    private static void cleanup(Context ctx) {
        try {
            ...
            throw new yueyang.vostok.data.exception.VKTxException("Failed to close connection", e);
        } finally {
            ctx.conn = null;
            ctx.depth = 0;
            ctx.rollbackOnly = false;
    ...
    private static void checkTimeout(Context ctx) {
        ...
        if (elapsed > timeout) {
            ctx.rollbackOnly = true;
            throw new yueyang.vostok.data.exception.VKTxException("Transaction timeout");
        }
    }
    ...
    private static void createSavepoint(Context ctx) {
        try {
            ...
            throw new yueyang.vostok.data.exception.VKTxException("Failed to create savepoint", e);
        }
    }
