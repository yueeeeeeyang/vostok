        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            for (EntityMeta em : metas) {
                ...
                if (!tableExists(meta, schema, table)) {
                    throw new yueyang.vostok.exception.VKMetaException("Table not found: " + table);
                }
                Set<String> columns = loadColumns(meta, schema, table);
                for (FieldMeta fm : em.getFields()) {
                    String col = fm.getColumnName().toLowerCase();
                    if (!columns.contains(col)) {
                        throw new yueyang.vostok.exception.VKMetaException("Column not found: " + table + "." + fm.getColumnName());
                    }
                }
            }
        } catch (Exception e) {
            ...
            throw new yueyang.vostok.exception.VKMetaException("DDL validation failed", e);
        }
    }
